...
def cadastro(request):
    ...
    if len(senha) < 6:
        messages.error(request, 'Senha curta, necessário 6 caracteres ou mais.')
        return render(request, 'accounts/cadastro.html')

    if len(usuario) < 6:
        messages.error(request, 'Nome de usuário curto, necessário 6 caracteres ou mais.')
        return render(request, 'accounts/cadastro.html')
    ...
    if User.objects.filter(username=usuario).exists():
        messages.error(request, 'Esse nome de usuário já está sendo utilizado')
        return render(request, 'accounts/cadastro.html')

    if User.objects.filter(email=email).exists():
        messages.error(request, 'Esse endereço e-mail está sendo utilizado.')
        return render(request, 'accounts/cadastro.html')
